      ...
      forkedRepos.forEach(
        (repo) =>
          (projectContainer.innerHTML += `
            ...
            <a class="project-links" href="${repo.html_url}" target="_blank">${
            repo.name
          }</a> <br><i>With default branch ${repo.default_branch}</i>
            <p class="project-info"><b>Created:</b> ${new Date(
              repo.created_at
            ).toDateString()}</>
            <p class="project-info"><b>Latest push:</b> ${new Date(
              repo.pushed_at
            ).toDateString()}</p>
            <p class="project-info" id="commits-${
              repo.name
            }"><b>Number of commits:</b> </p>
            </div>
            `)
      );
      ...
      .then((data) => {
        ...
        if (myPullRequests) {
          ...
          document.getElementById(
            `commits-${repo.name}`
          ).innerHTML = `No pull requests done by ${USER}`;
        }
      })
