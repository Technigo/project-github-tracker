        ...
        fetch(`https://api.github.com/repos/${USER}/${repo.name}/commits`)
          ...
          .then((data) => {
            ...
            for (let i = 0; i < formattedProjectName.length; i++) {
              formattedProjectName[i] =
                formattedProjectName[i][0].toUpperCase() +
                formattedProjectName[i].substr(1)
            }
      ...
      .then((data) => {
        ...
        const myPullRequests = data.find(
          (pull) =>
            ...
            (pull.user.login === 'Svempolin' && pull.number === 67)
        )
